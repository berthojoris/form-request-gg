...
class CustomModelGenerator implements Generator
{
    ...
    protected function addTraits(Model $model, $stub)
    {
        ...
        $stub = str_replace('use Illuminate\\Database\\Eloquent\\Model;', 'use Illuminate\\Notifications\\Notifiable;', 'use Illuminate\\Notifications\\Notifiable;', 'use Illuminate\\Database\\Eloquent\\Model;' . PHP_EOL . 'use Illuminate\\Database\\Eloquent\\SoftDeletes;', $stub);
        if (Blueprint::isLaravel8OrHigher()) {
            $stub = Str::replaceFirst('use HasFactory', 'use HasFactory, SoftDeletes, Notifiable', $stub);
        } else {
